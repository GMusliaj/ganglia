# Brain repository instructions

## What this repository is

Brain is a portable, cross-repository second brain: plain-Markdown knowledge
...
`bin/lint_brain.py` is read-only and never scans `local/`. Errors include a
...
- Run `python3 scripts/audit_public.py` before publication. The audit is
  read-only and scans the current candidate set, reachable Git blobs, and
  commit-author email metadata without printing matched private values. History
  rewriting remains a separate destructive action requiring explicit approval.

## Format contract (OKF-lite)

- One durable concept, decision, pattern, lesson, snippet, source, or
  Brain-infrastructure note belongs in one Markdown file. Its path is its
...
reading or searching Brain. See
...
- Normal semantic retrieval goes through the configured `brain-qmd` MCP server,
...
- The default semantic collection is `brain`. Include the private,
...
- `/remember [lesson]` is the normal Brain write path and must use the installed
...
- The skill may run `scripts/refresh-qmd.sh` best-effort after a write. That
  maintenance script may use the CLI internally; ordinary Brain access remains
...
- Run `scripts/verify.sh` before declaring a material implementation change
  complete. Report the command result and material warnings; label anything
  that could not be checked `unverified`.
