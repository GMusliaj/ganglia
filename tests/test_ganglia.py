...
reindex_module = load_module("ganglia_reindex", ROOT / "bin" / "reindex.py")
guard_module = load_module("ganglia_guard", ROOT / "scripts" / "guard_shared.py")
audit_module = load_module("ganglia_public_audit", ROOT / "scripts" / "audit_public.py")
lint_module = load_module("ganglia_lint", ROOT / "bin" / "lint_ganglia.py")
artifact_module = load_module(
    "ganglia_artifact_bundle", ROOT / "bin" / "artifact_bundle.py"
)
commit_module = load_module(
    "ganglia_commit_shared", ROOT / "scripts" / "commit_shared.py"
)
review_module = load_module(
    "ganglia_reduce_artifact_review", ROOT / "scripts" / "reduce_artifact_review.py"
)
approval_module = load_module(
    "ganglia_record_artifact_approval",
    ROOT / "scripts" / "record_artifact_approval.py",
)
artifact_eval_module = load_module(
    "ganglia_eval_artifacts", ROOT / "scripts" / "eval_artifacts.py"
)
canvas_module = load_module("ganglia_canvas", ROOT / "bin" / "canvas.py")
session_module = load_module(
    "ganglia_session_catalog", ROOT / "bin" / "sync_codex_sessions.py"
)
...
class GuardTests(unittest.TestCase):
    ...
    def test_github_runner_identity_is_treated_as_generic(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            document = root / "README.md"
            document.write_text("Use the repository runner for checks.\n")
            ...
            with (
                ...
                mock.patch.dict(
                    guard_module.os.environ,
                    {"LOGNAME": "runner", "USER": "runner"},
                ),
                mock.patch.object(
                    guard_module.socket,
                    "gethostname",
                    return_value="github-actions-host",
                ),
                mock.patch.object(guard_module, "command_value", return_value=""),
            ):
                failures = guard_module.scan(
                    root,
                    [document],
...
class ArtifactBundleTests(unittest.TestCase):
    ...
    def test_recall_rejects_invalid_shared_and_local_bundles_without_writes(self):
        ...
        cases = [
            ...
            ("payload escapes Ganglia root", escape_root),
...
class SharedCommitTests(unittest.TestCase):
    ...
    def ganglia_repository(self, root: Path):
        for folder in lint_module.SHARED_FOLDERS:
            (root / folder).mkdir(parents=True)
        (root / "meta").mkdir()
        (root / "meta" / "tag-taxonomy.md").write_text("", encoding="utf-8")
...
class CanvasTests(unittest.TestCase):
    ...
    def test_render_is_offline_and_contains_layer_controls(self):
        graph = {
            "nodes": [
                {
                    "id": "doc:ganglia:patterns/example.md",
                    "label": "Example",
                    "kind": "document",
                    "category": "pattern",
                    "collection": "ganglia",
            ...
            "links": [],
            "meta": {"collections": ["ganglia"], "scope": "shared"},
        }
        ...
        self.assertIn("Connected nodes form a ganglion with branching paths", rendered)
    ...
    def test_live_state_version_changes_when_a_watched_source_changes(self):
        graph = {"nodes": [], "links": [], "meta": {"collections": ["ganglia"], "scope": "shared"}}
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_private_scope_adds_codex_sessions_to_ganglia(self):
        available = {
            "ganglia": canvas_module.Collection("ganglia", ".", True),
        ...
        self.assertEqual([item.name for item in shared], ["ganglia"])
        self.assertEqual(
            [item.name for item in private], ["ganglia", "codex-sessions"]
        )
