#!/usr/bin/env python3
"""Render QMD-indexed Ganglia Markdown as an offline force-directed graph.

    bin/canvas.py                         # serve the shared graph with live reload
    bin/canvas.py --collection ganglia      # select a collection (repeatable)
...
def choose_collections(
    ...
    if "ganglia" in available:
        selected = [available["ganglia"]]
...
def render_library(graph: dict[str, object]) -> str:
    ...
    labels = {"ganglia": "Knowledge", "codex-sessions": "Codex sessions"}
    collection_rows = [
...
def render_html(
    ...
    logo_path = Path(__file__).parents[1] / "docs" / "assets" / "ganglia-mark.svg"
...
def main() -> int:
    ...
    # This normalizes an already-bound wildcard for the local browser URL; the
    # server bind itself is controlled by the explicit --host argument.
    browser_host = (
        "127.0.0.1" if ipaddress.ip_address(bound_host).is_unspecified else bound_host
    )
    ...
    watcher = threading.Thread(target=watch, name="ganglia-canvas-watch", daemon=True)
    watcher.start()
