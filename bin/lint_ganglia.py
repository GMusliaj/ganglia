#!/usr/bin/env python3
"""Validate shared Ganglia entries without mutating them."""
    ...
    for _, document in sorted(documents.items(), key=lambda item: str(item[0])):
        ...
        for target in MARKDOWN_LINK.findall(document.body):
            ...
            try:
                ...
                errors.append(f"{relative}: link escapes Ganglia root: {target}")
                continue
...
def main() -> int:
    ...
    for error in errors:
        print(f"error: {error}")
    print(f"Ganglia lint: {len(errors)} error(s), {len(warnings)} warning(s).")
