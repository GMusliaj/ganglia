#!/usr/bin/env python3
"""Commit the shared Ganglia allowlist without touching unrelated staged work."""
...
def commit_message(changed: list[str]) -> str:
    if len(changed) == 1:
        return f"ganglia: update {Path(changed[0]).stem}"
    return f"ganglia: update {len(changed)} knowledge files"


def commit_shared(root: Path) -> list[str]:
    root = root.resolve()
    descriptor, temporary_name = tempfile.mkstemp(prefix="ganglia-shared-index-")
    os.close(descriptor)
